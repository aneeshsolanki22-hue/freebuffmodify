...
export async function validateApiKey({
  ...
  // ponytail: no codebuff.com backend in this fork — every non-empty API key
  // is accepted locally. Revisit if server-backed auth is ever re-added.
  if (!apiKey) {
    logger.error('❌ API key validation failed - no API key')
    throw createAuthError('Invalid API key')
  }
  return {
    id: `local-${getApiKeyHash(apiKey).slice(0, 12)}`,
